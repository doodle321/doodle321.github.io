...
(function(){
  ...
  const WA_NUMBER = "923103694160";

  function setErr(input, bad){
    input.closest(".field").classList.toggle("error", bad);
    return !bad;
  }
  ...
  form.addEventListener("submit", e => {
    ...
    ok = setErr(contact, !contact.value.trim()) && ok;
    ok = setErr(message, message.value.trim().length < 5) && ok;
    if (!ok) return;
  ...
  ["cName", "cContact", "cMessage"].forEach(id =>
    document.getElementById(id).addEventListener("input", e => e.target.closest(".field").classList.remove("error")));
